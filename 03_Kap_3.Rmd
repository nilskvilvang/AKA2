...
```{r}
pacman::p_load(readxl, car, rgl, flextable, plotly, latex2exp, ggfortify, gridExtra, factoextra, corrplot, Directional, tidyverse, palmerpenguins, psych, paran, kableExtra, multiUS, xtable, GPArotation, EFAtools, nFactors, rstatix, calibrate, Matrix, codefolder)
```

```{r codefolder, echo=FALSE, results='asis'}
codefolder::bookdown(init = "show")
```
...
* de er metoder for data reduksjon
...
![Modifisert fra @bjerkanFaktoranalyse2007b, s. 225, fig. 10.1](PCA-EFA-CFA.png)
...
![Fra @bastosExploratoryFactorAnalysis2021](PCA-EFA.png){width=50%}
